...
const wireAPath = stringA.split(",");
const wireBPath = stringB.split(",");
...
const initialPosition: WirePosition = {
  x: 0,
  y: 0,
  distance: 0,
  steps: 0
...
const testPath1A = ["R8", "U5", "L5", "D3"];
const testPath1B = ["U7", "R6", "D4", "L4"];
const testPath2A = "R75,D30,R83,U83,L12,D49,R71,U7,L72".split(",");
const testPath2B = "U62,R66,U55,R34,D71,R55,D58,R83".split(",");
const testPath3A = "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51".split(",");
const testPath3B = "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7".split(",");

// Write a function that collects the positions based on the wire's moves
const getWirePositions = (previousPositions: WirePosition[], move: string) => {
  const previousPosition: WirePosition = [
    ...previousPositions
  ].pop() as WirePosition;
  ...
  const newPositions: WirePosition[] = [];
  const stepRange = range(1, steps);

  let newX: number = previousPosition.x;
  let newY: number = previousPosition.y;

  stepRange.forEach(step => {
    if (direction === "R" || direction === "L") {
      newX = previousPosition.x + (isPositive ? step : -step);
    } else {
      newY = previousPosition.y + (isPositive ? step : -step);
    }

    const newPosition: WirePosition = {
      x: newX,
      y: newY,
      distance: getDistance(newX, newY),
      steps: previousPosition.steps + step
    };
...
// Get both wires' positions
const test1APositions = testPath1A.reduce(getWirePositions, [initialPosition]);
const test1BPositions = testPath1B.reduce(getWirePositions, [initialPosition]);
const test2APositions = testPath2A.reduce(getWirePositions, [initialPosition]);
const test2BPositions = testPath2B.reduce(getWirePositions, [initialPosition]);
const test3APositions = testPath3A.reduce(getWirePositions, [initialPosition]);
const test3BPositions = testPath3B.reduce(getWirePositions, [initialPosition]);

const wireAPositions = wireAPath.reduce(getWirePositions, [initialPosition]);
const wireBPositions = wireBPath.reduce(getWirePositions, [initialPosition]);
// console.log({ test1Positions, test2Positions });

const getIntersections = (
  wireA: WirePosition[],
  wireB: WirePosition[]
): WirePosition[] => {
  const result = wireA.reduce(
    (intersections: WirePosition[], wireAPosition: WirePosition) => {
      if (wireAPosition.x === 0 && wireAPosition.y === 0) return intersections;

      const wireBPosition = wireB.find(
        wireBPosition =>
          wireBPosition.x === wireAPosition.x &&
          wireBPosition.y === wireAPosition.y
      );

      if (wireBPosition) {
        const newIntersection = {
          ...wireAPosition,
          steps: wireAPosition.steps + wireBPosition.steps
        };
...
// Now find the positions that exist in both arrays
// const intersections = getIntersections(test1APositions, test1BPositions);
// const intersections = getIntersections(test2APositions, test2BPositions);
// const intersections = getIntersections(test3APositions, test3BPositions);
const intersections = getIntersections(wireAPositions, wireBPositions);
...
const fewestStepsIntersection = intersections.sort(
  (a, b) => a.steps - b.steps
)[0];

console.log({ fewestStepsIntersection });
